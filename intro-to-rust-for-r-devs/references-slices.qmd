...
- we always know the length of a slice i.e. `.len()` is available
- denoted as `&[T]` (again remember `T` is a generic type)—e.g. `&[f64]` or `&[i32]`
...
- Rewrite `mean()` to use a slice of f64
