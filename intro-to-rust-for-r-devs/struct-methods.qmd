...
- Implement a `Point::new()` associated function that returns `Self` and has two arguments `x: f64` and `y: f64`
- Define a method `euclidean_distance()` which calculates the distance between itself and a `destination` returning an `f64`
