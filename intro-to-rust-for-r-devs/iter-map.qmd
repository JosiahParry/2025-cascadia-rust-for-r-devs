...
- Turbofish `::<Type>()` is used to explicitly specify generic types when needed
...
* `.map()` takes a closure that operates on each element.
* The closure argument uses vertical bars `| |` to enclose parameters.
* `.collect()` requires a type annotation if the target type cannot be inferred.
* Use turbofish syntax when needed, e.g. `.sum::<f64>()`.
