...
::: aside
These closures, too, are not subsettable.
:::
...
Or, we can specify it in
- we have to explicitly **collect** the result into our own type
  - use the `.collect()` method
- typically this is a `Vec<T>` that we want to collect to.
- we can specify the type explicitly by using `let x: Vec<f64> = x.iter().map().collect()`
